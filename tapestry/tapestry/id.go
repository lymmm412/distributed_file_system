...
func (id ID) BetterChoice(first ID, second ID) bool {
	...
	firstSPL := SharedPrefixLength(id, first)
	secondSPL := SharedPrefixLength(id, second)
	if firstSPL > secondSPL {
		return true
	} else if firstSPL < secondSPL {
		return false
	}
	firstID := first.String()
	secondID := second.String()
	for j, l := 0, len(firstID); j < l; j++ {
		if firstID[j] == secondID[j] {
			continue
		} else if firstID[j] > id.String()[j] && secondID[j] > id.String()[j] {
			if firstID[j] < secondID[j] {
				return true
			} else {
				return false
...
func (id ID) Closer(first ID, second ID) bool {
	// TODO: students should implement this
	idValue := id.big()
	firstValue := first.big()
	secondValue := second.big()
	var firstDiff big.Int
	var secondDiff big.Int
	firstDiff.Sub(firstValue, idValue)
	secondDiff.Sub(secondValue, idValue)
	res := firstDiff.CmpAbs(&secondDiff)
	if res < 0 {
		return true
	} else {
		return false
