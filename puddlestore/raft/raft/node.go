...
import (
	...
	"github.com/brown-csci1380/YimingLiBrown-YuqiChai-s19/cs138"
...
func CreateNode(localPort int, remoteAddr *RemoteNode, config *Config) (rp *RaftNode, err error) {
	...
	zkConn, _, err := zk.Connect(hosts, time.Second)
	if err != nil {
		return nil, err
	}
	isExit, _, err := zkConn.Exists("/raft")
	if !isExit {
		fmt.Printf("it should exist!")
		return nil, errors.New("something wrong in the puddleclient Create function")
	} else if err != nil {
		fmt.Printf("strange err!!!!!!!!!!!!!!!!!")
		return nil, err
	}
	acls := zk.WorldACL(zk.PermAll)

	_, err = zkConn.Create("/raft/raft"+r.Id, []byte(conn.Addr().String()), int32(1), acls)
	if err != nil {
		return nil, errors.New("fail to create a child in zookeeper")
	}
	// // }
	// child, _, err := zkConn.Children("/raft")
	// // length := len(child)
